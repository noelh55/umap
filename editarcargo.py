...
class EditarCargo(tk.Toplevel):
    def __init__(self, master, cargo_id=None, nombre=None, descripcion=None):
        ...
        # --- Cargar datos del cargo recibido ---
        if cargo_id:
    ...
    def actualizar(self):
        ...
        cargo_id = self.entry_id.get().strip()
        ...
        try:
            ...
            cur.execute(
                ...
                (nombre, descripcion, cargo_id)
            )
            conn.commit()
            conn.close()

            messagebox.showinfo("Éxito", f"Cargo con ID {cargo_id} actualizado correctamente.")
            self.destroy()
...
if __name__ == "__main__":
    ...
    EditarCargo(root, cargo_id=1, nombre="Gerente", descripcion="Responsable de la gestión general de la empresa.")
    root.mainloop()